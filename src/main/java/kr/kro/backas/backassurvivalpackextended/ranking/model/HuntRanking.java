...
public class HuntRanking extends AbstractRanking<IntegerRankData> {
    @Override
    ...
    public void send(Player player, int displayAmount, int page) {
        ...
        for (int rankIndex = (page - 1) * displayAmount;
            ...
            player.sendMessage(Component.text()
                    ...
                    .append(Component.text(data.getAmount() + " 마리", NamedTextColor.YELLOW))
            );
        }
        ...
        player.sendMessage(Component.text().append(
                ...
                Component.text("(" + getRankData(player).getAmount() + " 마리)", NamedTextColor.GRAY))
        );
    }
    ...
    protected void load() {
        for (OfflinePlayer player : Bukkit.getOfflinePlayers()) {
            ...
            if (data != null) data.setAmount(player.getStatistic(Statistic.MOB_KILLS));
            else {
